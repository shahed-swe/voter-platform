/*
 * Avro Phonetic (Roman → Bangla) transliteration.
 *
 * Vendored from jsAvroPhonetic (OmicronLab) by Rifat Nabi — MPL-1.1.
 * https://github.com/torifat/jsAvroPhonetic  (data + parser only; no DOM/deps).
 * Bundled locally so voter search can transliterate English typing to Bangla
 * offline (no CDN / CSP issues).
 */
const OmicronLab = {};
OmicronLab.Avro = {};
...
/** Roman → Bangla phonetic. Returns '' for empty input. */
export function toBanglaPhonetic(text) {
    if (!text) return '';
    try {
...
/**
 * Build voter-search terms from a raw input. When the input contains Latin
 * letters, also produce its Avro-phonetic Bangla form so the API can match
 * Bangla names typed in English. Bangla/numeric input passes through unchanged.
 */
export function voterSearchTerms(input) {
    const q = (input || '').trim();
    if (!q) return { search: undefined, search_bn: undefined };
    return {
        search: q,
        search_bn: HAS_LATIN.test(q) ? toBanglaPhonetic(q) : undefined,
    };
}
